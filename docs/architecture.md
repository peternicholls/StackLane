...
| Lifecycle orchestration | `core/lifecycle` | Sequences the 11 steps of `up`, plus down/attach and shutdown cleanup. Wraps every operator-visible failure in `StepError`. |
| Docker SDK | `infra/docker` | Engine SDK only: networks, container queries, healthcheck waiting, log streams. NOT `docker compose`. |
| Compose subprocess | `infra/compose` | The single owner of `docker compose ...` invocation against the resolved stack compose file (currently `stacks/20i/docker-compose.20i.yml`). |
